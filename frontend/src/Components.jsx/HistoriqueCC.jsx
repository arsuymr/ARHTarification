...
export default function HistoriqueCC() {
    ...
    useEffect(() => {
        const fetchUnits = async () => {
            if (selectedOperator) {
                try {
                    console.log("coocoou", selectedOperator)
            ...
            <div className="flex flex-col w-full">
                ...
                {classes.map((classe) => (
                    <div key={classe.ID} className="mt-8">
                        ...
                        <TabAffichage IDClasse={classe.ID} selectedYear={selectedYear} selectedUnit={selectedUnit} selectedOperator={selectedOperator} />
                    </div>
                ))}
            </div>
        </div>
