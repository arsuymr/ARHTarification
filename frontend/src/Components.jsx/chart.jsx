...
export default function Charts() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await axios.post('http://127.0.0.1:5000/resultat/Stats', {
                    "NomUnity": nomUnity,
    ...
    return (
        <div className='flex'>
            ...
            <div className="m-2 w-[2560px]  ">
                <div className='p-4 pt-2'><TabDashboard /></div>
                        ...
                        <>
                            <div className="flex">
                                <LineChart
                                    ...
                                    height={300}
                                    series={[
                                        { data: data1, yAxisKey: 'leftAxisId', color: 'black', label: 'Tarifs Liquifaction  [DA/1000 Sm3]' },
                                    ]}
                                    xAxis={[{ scaleType: 'point', data: xLabels }]}
                                    yAxis={[
                                        {
                                            id: 'leftAxisId',
                                            min: Math.min(...data1) > 0 ? Math.min(...data1) - 0.1 * Math.min(...data1) : 0,
                            ...
                            <div className="flex">
                                <LineChart
                                    ...
                                    height={300}
                                    series={[
                                        { data: data2, label: 'Tarif Separation  [DA/1000 Tonne]', yAxisKey: 'leftAxisId', color: "#114021" },
                                    ]}
                                    xAxis={[{ scaleType: 'point', data: xLabels }]}
                                    yAxis={[
                                        {
                                            id: 'leftAxisId',
                                            min: Math.min(...data2) > 0 ? Math.min(...data2) - 0.1 * Math.min(...data2) : 0,
                        ...
                        <>
                            <div className="flex-1">
                                <LineChart
                                    ...
                                    height={300}
                                    series={[
                                        { data: data1, label: 'Creu de Liquifaction Par Tonne', yAxisKey: 'leftAxisId' },
                                        { data: data2, label: 'Creu Liquifaction Par Sm3', yAxisKey: 'leftAxisId' },
                                    ]}
                                    xAxis={[{ scaleType: 'point', data: xLabels }]}
                                    yAxis={[
                                        {
                            ...
                            <div className="flex-1">
                                <LineChart
                                    ...
                                    height={300}
                                    series={[
                                        { data: data3, label: 'Creu Separation Par tonne', yAxisKey: 'leftAxisId' },
                                        { data: data4, label: 'Creu Separation Par Sm3', yAxisKey: 'leftAxisId' },
                                    ]}
                                    xAxis={[{ scaleType: 'point', data: xLabels }]}
                                    yAxis={[
                                        {
                                            id: 'leftAxisId',
                                            min: Math.min(...data3) > 0 ? Math.min(...data3) - 0.1 * Math.min(...data3) : 0,
                                            max: Math.max(...data3) + 0.1 * Math.max(...data3),
                                        },
                                        {
                                            id: 'rightAxisId',
                                            min: Math.min(...data4) > 0 ? Math.min(...data4) - 0.1 * Math.min(...data4) : 0,
